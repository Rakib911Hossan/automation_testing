...
export class ClientPage extends BasePage {
  ...
  readonly clientConnectivityTypeDropdown: Locator;
  readonly saveClientButton: Locator;
  readonly confirmButton: Locator;

  constructor(page: Page) {
    super(page);
    this.clientMenu = page.getByText('Client', { exact: true });
    ...
    this.clientConnectivityTypeDropdown = page.locator('div').filter({ hasText: /^Client Connectivity Type\*Select \.\.\.$/ }).locator('div').nth(4);
  ...
  async navigateToClientPage() {
    await this.page.goto(process.env.CLIENT_PAGE_URL as string);
  }
  ...
  async addClient(data: {
    ...
    await this.clientConnectivityTypeDropdown.click();
    await this.page.getByText('Share', { exact: true }).click();

    await this.saveClientButton.click();
    await this.confirmButton.click();
    await this.page.waitForTimeout(2000); // Wait for the client to be added, adjust as necessary
  }
}
